...
import { databases, DATABASE_ID, COLLECTION_ID_RT03, getUserRole } from '../services/appwriteConfig';
import { Query, ID } from 'appwrite';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
export default function RT03Billing() {
    ...
    const handleSaveRecord = async (e) => {
        ...
        try {
            if (isEditing) {
                ...
                await databases.updateDocument(DATABASE_ID, COLLECTION_ID_RT03, isEditing, {
                    Nama: namaWarga,
                ...
                if (existingResident) {
                    ...
                    await databases.updateDocument(DATABASE_ID, COLLECTION_ID_RT03, existingResident.$id, {
                        [bulanPilih]: Number(nominal),
                    });
                    ...
                    await databases.createDocument(DATABASE_ID, COLLECTION_ID_RT03, ID.unique(), {
                        Nama: namaWarga,
                        [bulanPilih]: Number(nominal),
                    });
    ...
    const handleDelete = async (id, nama) => {
        if (window.confirm(`YAKIN HAPUS? Seluruh record data milik ${nama} pada baris ini akan dihapus permanen.`)) {
            try {
                await databases.deleteDocument(DATABASE_ID, COLLECTION_ID_RT03, id);
